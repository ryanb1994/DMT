...
for i in range(0, len(df)):
	if contains_word(df['What programme are you in?'][i].lower(), artificial_intelligence) == True or 'artificial' in df['What programme are you in?'][i].lower() or 'ai' in df['What programme are you in?'][i].lower():
		...
		s.loc[i] = 'AI'
	elif contains_word(df['What programme are you in?'][i].lower(), bioinformatics) == True or 'bio' in df['What programme are you in?'][i].lower():
		s = df['What programme are you in?']
		s.loc[i] = 'BIO'
		...
		s = df['What programme are you in?']
		s.loc[i] = 'BA'
	elif contains_word(df['What programme are you in?'][i].lower(), computer_science) == True or 'computer' in df['What programme are you in?'][i].lower() or 'big' in df['What programme are you in?'][i].lower():
		s = df['What programme are you in?']
		s.loc[i] = 'CS'
