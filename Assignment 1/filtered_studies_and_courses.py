...
df = df.iloc[:, :-11]
df.drop(['Unnamed: 0', 'Unnamed: 0.1', 'Timestamp'], axis=1, inplace = True)

for i in range(0, len(df)):
	if df['Have you taken a course on machine learning?'][i] == 'unknown':
		df.drop(i, inplace = True)
		continue
	if df['Have you taken a course on statistics?'][i] == 'unknown':
		df.drop(i, inplace = True)
		continue
	if df['Have you taken a course on databases?'][i] == 'unknown':
		df.drop(i, inplace = True)
		continue
	if df['Have you taken a course on information retrieval?'][i] == 'unknown':
		df.drop(i, inplace = True)
		continue
